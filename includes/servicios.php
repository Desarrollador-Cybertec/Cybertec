...
<section id="servicios" class="py-20 lg:py-28 relative">
    <div class="container mx-auto px-4 sm:px-6">
        <div class="text-center mb-10 sm:mb-16 max-w-3xl mx-auto">
            ...
            <p class="text-slate-400 text-base sm:text-lg" data-aos="fade-up" data-aos-delay="100">Cuatro pilares de soluciones diseñadas para escalar con su negocio, garantizando seguridad, velocidad y fiabilidad en cada capa.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-6">
            ...
            <div class="relative group rounded-3xl overflow-hidden glass-panel glass-card-hover <?php echo $s['span']; ?>" data-aos="fade-up" data-aos-delay="<?php echo $index * 100; ?>">
                ...
                <!-- Gradient Overlay -->
                <div class="absolute inset-0 bg-gradient-to-t <?php echo $s['bg_gradient']; ?> opacity-60"></div>
                
                <!-- Content -->
                <div class="relative <?php echo (strpos($s['span'], 'lg:col-span-3') !== false) ? 'p-6 sm:p-8 lg:px-12 lg:py-6' : 'p-6 sm:p-8'; ?> h-full flex flex-col justify-between z-10">
                    <div>
                        <div class="w-12 sm:w-16 h-12 sm:h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-2xl sm:text-3xl mb-3 sm:mb-4 backdrop-blur <?php echo $s['color']; ?>">
                            <i class="<?php echo $s['icon']; ?>"></i>
                        </div>
                        
                        <h3 class="text-2xl sm:text-3xl font-display font-bold text-white mb-1"><?php echo $s['titulo']; ?></h3>
                        <p class="<?php echo $s['color']; ?> font-medium tracking-wide mb-3 sm:mb-4 uppercase text-xs sm:text-sm"><?php echo $s['subtitulo']; ?></p>
                        
                        <ul class="space-y-2 mb-6">
                            <?php foreach ($s['puntos'] as $punto): ?>
                            <li class="flex items-start gap-3 text-slate-300 text-sm leading-tight">
